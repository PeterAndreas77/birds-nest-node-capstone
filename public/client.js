...
function renderFlightPlans(plans) {
  ...
  for (let index in plans) {
    ...
    myFlightPlans += `<div class="log-item" plan-id="${plans[index].id}">
  ...
  $.ajax({
    ...
    .done(result => {
      if (result == undefined || result.length == 0) {
        $("#flight-plans-view").html(
          "No plans in that country or city were found!"
        );
      } else {
...
function searchFlightLogs(place, author) {
  $.ajax({
    ...
    .done(result => {
      if (result == undefined || result.length == 0) {
        $("#flight-logs-view").html(
          "No logs in that country or city were found!"
        );
      } else {
...
function renderFlightLogs(logs) {
  ...
  for (let index in logs) {
    ...
    myFlightLogs += `<div class="log-item" log-id="${logs[index].id}">
            <p>${logs[index].title}</p>
            <p>( ${feathers} )</p>
            <p>${logs[index].duration} days in ${logs[index].city}, ${
      logs[index].country
    }</p>
...
$(document).ready(function() {
  ...
  $("#flight-plans-view").on("click", ".edit-btn", event => {
    let updPlanID = $(event.currentTarget)
      .closest(".log-item")
      .attr("plan-id");
  ...
  $("#flight-plans-view").on("click", ".del-btn", event => {
    let delPlanID = $(event.currentTarget)
      .closest(".log-item")
      .attr("plan-id");
  ...
  $("#flight-logs-view").on("click", ".log-btn", event => {
    let newLogID = $(event.currentTarget)
      .closest(".log-item")
      .attr("plan-id");
  ...
  $("#flight-logs-view").on("click", ".del-btn", event => {
    let delLogID = $(event.currentTarget)
      .closest(".log-item")
      .attr("log-id");
